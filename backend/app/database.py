...
def init_db():
    """Verify database connection. DDL is managed externally on shared DB."""
    with db() as conn:
        conn.execute("SELECT 1")
    print("[database] MySQL 连接成功")
...
def init_mysql_tables():
    ...
    with db() as conn:
        try:
            # 作业表（支持截止规则，late_policy: allow/deny/penalty）
            conn.execute("""
                CREATE TABLE IF NOT EXISTS assignments (
                    ...
                    late_policy ENUM('allow', 'deny', 'penalty') DEFAULT 'deny',
            ...
            conn.execute("""
                CREATE TABLE IF NOT EXISTS submissions (
                    ...
                    allow_resubmit_override TINYINT,
                    submitted_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                    INDEX idx_assignment_student (assignment_id, student_user_id)
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4
            """)

            print("[init] MySQL 作业模块表初始化成功")
        except Exception as e:
            print(f"[init] MySQL 作业模块表初始化警告: {e}")
