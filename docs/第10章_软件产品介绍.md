...
| `postgres` | `postgres:16-alpine` | 仅内网（5432） | 统一开发数据库，存储全部业务数据 |
| `redis` | `redis:7-alpine` | 仅内网（6379） | 会话缓存、事件队列（Reflex 实时推送） |
| `pgadmin` | `dpage/pgadmin4` | 5050（管理员专用，按需启动） | 教师 DDL 操作与数据库可视化管理 |
...
| `postgres-preview-{pr}` | 隔离的测试数据库，预填 fixture 数据 |
...
services:
  oaepp-app:
    ...
    environment:
      DATABASE_URL: postgresql://oaepp:${POSTGRES_PASSWORD}@postgres:5432/oaepp_dev
          ...
          curl -X POST "${{ secrets.COOLIFY_API_URL }}/api/v1/applications" \
            ...
            -d '{
              ...
              "environment_variables": {
                "DATABASE_URL": "postgresql://oaepp:preview@postgres-preview-${{ github.event.number }}:5432/oaepp_preview",
                "REFLEX_ENV": "dev"
              }
            }'
...
| 数据卷 | `postgres_data` 挂载至 Coolify 持久卷，防止重部署丢失数据 |
...
services:
  postgres-preview-shared:
    ...
    volumes:
      - preview_db_data:/var/lib/postgresql/data

  oaepp-preview-${PR_NUMBER}:
    build: .
    environment:
      DATABASE_URL: postgresql://preview:${PREVIEW_DB_PASSWORD}@postgres-preview-shared:5432/oaepp_preview
