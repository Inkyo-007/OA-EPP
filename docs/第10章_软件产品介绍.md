...
## 17. 学生 GitHub Commit 规范

本节规定《工程实践 4》项目中，学生提交代码时必须遵守的 Commit 规范。规范基于 [Conventional Commits](https://www.conventionalcommits.org/zh-hans/) 标准，并针对课程场景进行简化。
...
课程提供两种开发方式，**推荐方式 A**（服务器直接开发）：

| 方式 | 说明 | 推荐 |
|------|------|------|
| **A：Remote SSH 连接课程服务器** | 直接在课程服务器上开发，数据库已就绪，无需本机配置 Python/Node | ✅ 推荐 |
| **B：本机开发 + 远程数据库** | 本机安装 Python/Node，连接课程共享数据库 | 可选 |

**方式 A 操作步骤：**

1. 在 VS Code 中安装 `ms-vscode-remote.remote-ssh` 插件
2. 按 `Ctrl+Shift+P` → 输入 `Remote-SSH: Add New SSH Host`
3. 输入教师提供的 SSH 连接命令：`ssh 学号@课程服务器IP`
4. 连接成功后，用 VS Code 打开 `/home/学号/` 目录
...
编辑 `.env`，填写教师提供的数据库连接信息：

```ini
# .env - 本地开发环境变量
DATABASE_URL=postgresql://oaepp_dev:密码@课程服务器IP:5432/oaepp_dev

# 如使用 Redis（可选）
REDIS_URL=redis://课程服务器IP:6379/0
```

> 数据库账号密码由教师统一发放，每位学生使用相同的开发数据库，通过 schema 或表前缀隔离。
...
遵循 Section 17 的 Commit 规范，完整工作流如下：
...
# Step 2：创建功能分支
git checkout -b feat/学号-功能描述
# 示例：git checkout -b feat/2023001-add-assignment-submit
...
| 数据库连接失败 | 确认 `.env` 中 `DATABASE_URL` 正确，课程服务器防火墙已开放 5432 端口 |
| 端口 3000 被占用 | `reflex run --frontend-port 3001` |
| `pytest` 找不到模块 | 确保在项目根目录执行，且虚拟环境已激活 |
| 推送被拒绝（non-fast-forward） | `git fetch origin && git rebase origin/main` 后重试 |
